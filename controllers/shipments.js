...
export const getShipmentsByOrganizationId = async (req, res) => {
    ...
    try {
        const shipments = await Shipment.find({ organization: organizationId }).populate('organization');
        if (!shipments) {
            return res.status(404).json({ message: 'No shipments found for the organization' });
        }
        res.status(200).json(shipments);
    } catch (error) {
        res.status(500).json({ message: error.message });
