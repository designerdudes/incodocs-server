...
export const addOrganization = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(404).json({ message: `User with ID ${ownerId} not found` }); // Return 404 if user with the given ID is not found
        }
...
export const updateOrganization = async (req, res) => {
    ...
    try {
        const updatedOrganization = await Organization.findByIdAndUpdate(id, { name, description, owner, members, address }, { new: true });
...
export const getOrganizationById = async (req, res) => {
    ...
    try {
        const organization = await Organization.findById(id).populate('owner members');
